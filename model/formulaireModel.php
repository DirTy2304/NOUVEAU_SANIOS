 ...
 * @return header
 */
function updateVehicule($Emploi, $alias, $societe, $marque, $modele, $type, $plaqueImmatriculation, $datemisecirculation, $cession)
{
    $bd = connectBd();
    ...
    SET `plaque` = :immatriculation , `fk_marque` = :idMarque, `fk_modele` = :idModele, `fk_type` = :idType, `fk_societe` = :idSociete, 
     `date_mise_circulation` = :dateMiseCirculation, `cession` = :cession 
    WHERE `ancien_id` = :alias");

    $idMarque = $marque['id_marque']; // Extract the value from array
    $idModele = $modele['id_modele']; // Extract the value from array
    $idType = $type['id_type']; // Extract the value from array

    $sql->bindParam('alias', $alias);
    $sql->bindParam('idSociete', $societe);
    $sql->bindParam('immatriculation', $plaqueImmatriculation);
    $sql->bindParam('idMarque', $idMarque);
...
function insertIntoVehicule($Emploi, $societe, $marque, $modele, $type, $immatriculation, $inventaire, $dateMiseCirculation, $dateControleTechnique, $kilometrage, $alias){
    ...
    (`id_vehicule`,`plaque`, `fk_marque`, `fk_modele`, `fk_type`, `fk_societe`, `date_mise_circulation`, `date_controle_technique`, `kilometrage`, `cession`, `ancien_id`)
     VALUES  (NULL, :immatriculation, :marque, :modele, :typev, :societe,  :dateCirculation, :dateControleTechnique,  :kilometrage, '0', :alias);");
    ...
    $sql->bindParam('marque', $idMarque);
    $sql->bindParam('modele', $idModele);
    $sql->bindParam('typev', $idType);
